...
class Main {
    public String solution(String str) {
        ...
        char[] arr = str.toCharArray();  //String을 char 배열로
        int lt = 0;
        int rt = arr.length - 1;
        char tmp;

//for (rt = strArr.length - 1; rt > lt; rt--) {
        while (lt < rt) {
            if (Character.isAlphabetic(arr[lt])) {
                if (Character.isAlphabetic(arr[rt])) {  //[lt],[rt] 다 알파벳일경우 swqp
                    tmp = arr[lt];
                    arr[lt] = arr[rt];
                    arr[rt] = tmp;
                    lt++;
                    rt--;
                } else { //[lt]는 알파벳인데 [rt]는 아닐경우 rt만 감소
                    rt--;
                }
            } else if (Character.isAlphabetic(arr[rt])) { //[lt]는 알파벳이아니고 [rt]만 알파벳일 경우 lt만 하나증가
                lt++;
            } else {
                lt++;
...
- 코드2 : 이거 썼는데도 안풀림.. 둘다 알파벳일때만 바꾸는건지 하나는 알파벳이고 하나는 아니면 아닌걸 바꿔서 둘다 알파벳일때까지 기다리는건지 문제 
